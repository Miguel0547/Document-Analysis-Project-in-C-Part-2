	...
	int status = 0;
        if (p_list)
        {
            if (word && *word)
            {
                ...
                {
                    status = add_node_at_head(p_list, word);
                }
                else
                {
                    status = find_word(p_list, word);
                    if (status == 0)
                    {
                        status = add_node_after_current(p_list, word);
                    }
                    else
                    {
...
int read_file( struct linked_list *p_list, char *file_name )
{
	...
	for ( one_char = fgetc( input_file ) ; one_char != EOF ; one_char = fgetc( input_file ) )
	{
		...
		if ( one_char != 44 && one_char != 46 && isspace( one_char ) == 0 )
		{
		...
		{
			if ( index != 0 && isspace( buffer [ index - 1 ] ) == 0  )
			{
				buffer [index] = '\0' ;
...
struct word_entry get_prev_word( struct linked_list *p_list )
{
	...
	if ( p_list )
	{
		if ( p_list->p_head )
		{
			if ( p_list->p_current->p_previous )
			{
...
int write_unique_word_list_to_csv_file(  struct linked_list *p_list, char *file_name )
{
	...
	if ( out_file )
	{
		...
		while ( p_list->p_current )
		{
			word = p_list->p_current->one_word.unique_word ;
			strcat(word, "," ) ;
			sprintf ( word_count, "%d", p_list->p_current->one_word.word_count ) ;
			strcat( word_count,"\n") ;
			strcat( word, word_count ) ;
			fputs( word, out_file ) ;
			p_list->p_current = p_list->p_current->p_next ;
		}
